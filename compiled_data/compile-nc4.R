...
setwd(homedir)
shapefile<-st_read('peru_shp/per_admbnda_adm3_2018.shp')
output<- shapefile[c("IDDIST","IDDPTO","NOMBDEP","IDPROV","NOMBPROV","NOMBDIST")] # dataframe with names and IDs of region, provinces and district in rows. 
...
for (i in 1:length(list_files)){
  ...
  to_merge<-x@data[c("IDDIST","IDDPTO","NOMBDEP","IDPROV","NOMBPROV","NOMBDIST",'layer')]
  names(to_merge)[names(to_merge) == "layer"] <- name_temp
  output<-merge(output, to_merge, by=c("IDDIST","IDDPTO","NOMBDEP","IDPROV","NOMBPROV","NOMBDIST"))
  print(i)
}
